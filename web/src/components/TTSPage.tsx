...
import { Regenerate, Shuffle } from "./ui/Icons";
...
import ChatterboxVoices from "./ChatterboxVoices";
...
const Board = () => {
  ...
  const engine = appStore.useState((state) => state.engine);
  const streaming = appStore.useState((state) => state.streaming);
      ...
      <div className="flex flex-row mb-3">
        <Block title="Engine">
          <div className="flex gap-3">
            {(["kokoro", "chatterbox"] as const).map((eng) => (
              <Button
                key={eng}
                color={engine === eng ? "primary" : "default"}
                selected={engine === eng}
                onClick={() => {
                  appStore.setState((draft) => {
                    draft.engine = eng;
              ...
              >
                <span className="uppercase text-xs font-semibold">
                  {eng === "kokoro" ? "Kokoro (82M)" : "Chatterbox Turbo (350M)"}
                </span>
              </Button>
            ))}
            {engine === "chatterbox" && (
